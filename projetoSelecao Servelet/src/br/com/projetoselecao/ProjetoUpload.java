...
public class ProjetoUpload extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//Verifica se foi ou não adicionado um arquivo no campo selecione
		boolean instancia = ServletFileUpload.isMultipartContent(request);
		if(!instancia) {
			return;
		}
		
		//Solicitação de upload do arquivo
		...
		try {
			...
			if(!item.hasNext()){
				out.println("Arquivo não carregado!");
				return;
			}
			while (item.hasNext()) {
